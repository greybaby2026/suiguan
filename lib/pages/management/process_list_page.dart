...
class _ProcessListPageState extends State<ProcessListPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('工序管理')),
      body: _isLoading
  ...
  Widget _buildCard(Map<String, dynamic> item) {
    ...
    final unitPrice = safeToDouble(item['piece_price'] ?? item['price_per_unit'] ?? item['unit_price']);
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.settings_outlined, color: AppTheme.successColor, size: 22),
          ),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    Text('单价: ¥${unitPrice.toStringAsFixed(2)}/${unit.isNotEmpty ? unit : "件"}', style: const TextStyle(fontSize: 12, color: AppTheme.primaryColor, fontWeight: FontWeight.w600)),
                  ],
                ),
              ],
            ),
  ...
  void _showFormDialog({Map<String, dynamic>? item}) {
    ...
    final unitPriceCtrl = TextEditingController(text: (item?['piece_price'] ?? item?['price_per_unit'] ?? item?['unit_price'] ?? '')?.toString() ?? '');
        ...
        content: SingleChildScrollView(
          child: SizedBox(
            ...
            child: Column(
              ...
              children: [
                TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: '名称 *', prefixIcon: Icon(Icons.settings_outlined))),
                ...
                const SizedBox(height: 12),
                TextField(controller: unitPriceCtrl, decoration: const InputDecoration(labelText: '单价', prefixIcon: Icon(Icons.attach_money_outlined)), keyboardType: const TextInputType.numberWithOptions(decimal: true)),
                const SizedBox(height: 12),
                TextField(controller: descCtrl, decoration: const InputDecoration(labelText: '描述', prefixIcon: Icon(Icons.description_outlined)), maxLines: 3),
              ],
            ),
          ),
        ),
            ...
            onPressed: isSubmitting ? null : () async {
              ...
              try {
                final data = <String, dynamic>{
                  ...
                  'piece_price': unitPriceCtrl.text.trim().isEmpty ? null : double.tryParse(unitPriceCtrl.text.trim()),
                  'description': descCtrl.text.trim().isEmpty ? null : descCtrl.text.trim(),
                };
                if (isEdit) {
                  await _api.put('${ApiConfig.processes}/${item['id']}', data: data);
                } else {
                  await _api.post(ApiConfig.processes, data: data);
                }
                if (mounted) {
                  Navigator.pop(ctx);
  ...
  void _showDeleteConfirm(Map<String, dynamic> item) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('确认删除工序 ${item['name'] ?? ''}？此操作不可撤销。'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('取消')),
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
              try {
                await _api.delete('${ApiConfig.processes}/${item['id']}');
                if (mounted) {
  ...
  Widget _buildEmptyView() {
    return Center(child: Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        const Icon(Icons.settings_outlined, size: 48, color: AppTheme.textHint),
        const SizedBox(height: 8),
        const Text('暂无工序数据', style: TextStyle(color: AppTheme.textHint)),
        const SizedBox(height: 8),
        Text('点击右下角按钮新增工序', style: TextStyle(color: AppTheme.textHint.withOpacity(0.7), fontSize: 13)),
      ],
    ));
  }
