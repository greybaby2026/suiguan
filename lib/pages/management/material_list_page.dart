...
class _MaterialListPageState extends State<MaterialListPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final response = await _api.get(ApiConfig.materials);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('原料管理')),
      body: _isLoading
  ...
  Widget _buildCard(Map<String, dynamic> item) {
    ...
    final stock = safeToDouble(item['stock'] ?? item['current_stock'] ?? item['stock_qty']);
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.science_outlined, color: AppTheme.accentColor, size: 22),
          ),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    if (unit.isNotEmpty) ...[const SizedBox(width: 12), Text('单位: $unit', style: const TextStyle(fontSize: 12, color: AppTheme.textHint))],
                    const SizedBox(width: 12),
                    Text('库存: ${stock.toStringAsFixed(0)}', style: const TextStyle(fontSize: 12, color: AppTheme.primaryColor, fontWeight: FontWeight.w600)),
                  ],
                ),
              ],
            ),
        ...
        content: SingleChildScrollView(
          child: SizedBox(
            ...
            child: Column(
              ...
              children: [
                TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: '名称 *', prefixIcon: Icon(Icons.science_outlined))),
          ...
          ElevatedButton(
            onPressed: isSubmitting ? null : () async {
              ...
              try {
                ...
                if (isEdit) {
                  await _api.put('${ApiConfig.materials}/${item['id']}', data: data);
                } else {
                  await _api.post(ApiConfig.materials, data: data);
                }
                if (mounted) {
                  Navigator.pop(ctx);
  ...
  void _showDeleteConfirm(Map<String, dynamic> item) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('确认删除原料 ${item['name'] ?? ''}？此操作不可撤销。'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('取消')),
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
              try {
                await _api.delete('${ApiConfig.materials}/${item['id']}');
                if (mounted) {
  ...
  Widget _buildEmptyView() {
    return Center(child: Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        const Icon(Icons.science_outlined, size: 48, color: AppTheme.textHint),
        const SizedBox(height: 8),
        const Text('暂无原料数据', style: TextStyle(color: AppTheme.textHint)),
        const SizedBox(height: 8),
        Text('点击右下角按钮新增原料', style: TextStyle(color: AppTheme.textHint.withOpacity(0.7), fontSize: 13)),
      ],
    ));
  }
